...
class Jogo:
        ...
        if self.status == "Jogando":
            ...
            if self.cedula_selecionada:
                movimento = self.is_movimento_valido(
                    self.jogadores[turno], self.cedula_selecionada, linha, coluna
                )
                if movimento[0]:
                    self.jogar(
                        self.jogadores[turno],
                    ...
                    linha == self.cedula_selecionada[0]
                    and coluna == self.cedula_selecionada[1]
                ):
                ...
                if (
                    self.matriz_jogadores[linha][coluna].lower()
                    == self.jogadores[turno]
                ):
                    self.cedula_selecionada = [linha, coluna]

    # VERIFICANDO SE UM MOVIMENTO REALIZADO PELO JOGADOR É VÁLIDO
    def is_movimento_valido(
        self, jogador, localizacao_cedula, linha_destino, coluna_destino
    ):
        ...
        if obrigatorios != {}:
            if (linha_originaria, coluna_originaria) not in obrigatorios:
                return False, None
            elif [linha_destino, coluna_destino] not in obrigatorios[
                (linha_originaria, coluna_originaria)
            ]:
                return False, None
            ...
            if pulo:
                if len(pulo) == 1:
                    ...
                    for i in range(len(pulo)):
                        if (
                            abs(pulo[i][0] - linha_destino) == 1
                            and abs(pulo[i][1] - coluna_destino) == 1
                        ):
                            return True, pulo[i]
    ...
    def movimento_obrigatorio(self, localizacao_cedula):
        ...
        if (
            self.matriz_jogadores[l][c].islower()
            and self.matriz_jogadores[l][c] == jogador
            and self.turno % 2 == index
        ):
            if l > 0:
                if c < 7:
            ...
            self.matriz_jogadores[l][c].isupper()
            and self.matriz_jogadores[l][c] == jogador.upper()
            and self.turno % 2 == index
        ):

            if (
                not self.pulando
                and (jogador.lower() == "x" and l != 7)
                or (jogador.lower() == "o" and l != 0)
            ):
                conta_linha = l
                conta_coluna = c
                while True:
                    if conta_linha - 1 < 0 or conta_coluna - 1 < 0:
                        ...
                        if (
                            self.matriz_jogadores[conta_linha - 1][conta_coluna - 1]
                            not in array
                        ):
                            l_x = conta_linha - 1
                            l_c = conta_coluna - 1
                                ...
                                if self.matriz_jogadores[l_x - 1][l_c - 1] == "-":
                                    ...
                                    while True:
                                        if l_x - 1 < 0 or l_c - 1 < 0:
                                            ...
                                            if (
                                                self.matriz_jogadores[l_x - 1][l_c - 1]
                                                == "-"
                                            ):
                                                obrigatorios.append([l_x - 1, l_c - 1])
                                            else:
            ...
            if self.matriz_jogadores[linha_atual][coluna_atual].islower():
                if self.matriz_jogadores[linha_atual][coluna_atual] == "o":
                    if linha_atual > 0:
                        if coluna_atual < 7:
                            if (
                                self.matriz_jogadores[linha_atual - 1][coluna_atual + 1]
                                == "-"
                            ):
    ...
    def jogar(self, jogador, localizacao_cedula, linha_destino, coluna_destino, pulo):
        ...
        if (jogador == "x" and linha_destino == 7) or (
    ...
    def verifica_vencedor(self):
        ...
        x = sum(
            [
                contador.count("x") + contador.count("X")
                for contador in self.matriz_jogadores
            ]
        )
        o = sum(
            [
                contador.count("o") + contador.count("O")
                for contador in self.matriz_jogadores
            ]
        )
    ...
    def desenha(self):
        ...
        if self.cedula_selecionada:
            ...
            if obrigatorios != {}:
                if (
                    self.cedula_selecionada[0],
                    self.cedula_selecionada[1],
                ) not in obrigatorios:
                    x_vermelho = ALTURA / 8 * self.cedula_selecionada[1]
                    y_vermelho = ALTURA / 8 * self.cedula_selecionada[0]

                    pygame.draw.rect(
                        display, VERMELHO_CLARO, (x_vermelho, y_vermelho, 75, 75)
                    )
                else:
                    if movs[0] == []:
                        x_vermelho = ALTURA / 8 * self.cedula_selecionada[1]
                        ...
                        for i in range(len(movs[0])):
                            ...
                            pygame.draw.rect(
                                display, VERDE_CLARO, (x_possivel, y_possivel, 75, 75)
                            )
            else:
                if self.pulando:
                    x_vermelho = ALTURA / 8 * self.cedula_selecionada[1]
        ...
        if self.status != "Game Over":
            ...
            surface_texto, rect_texto = text_objects(
                "Vermelho: " + str(12 - o), fonte, VERMELHO_CLARO
            )
            rect_texto.center = (650, 30)
            display.blit(surface_texto, rect_texto)

            surface_texto, rect_texto = text_objects(
                "Branco: " + str(12 - x), fonte, BRANCO
            )
            ...
            if self.turno % 2 == 1:
                surface_texto, rect_texto = text_objects(
                    "Turno do branco", fonte, BRANCO
                )
                rect_texto.center = (700, ALTURA / 2)
                display.blit(surface_texto, rect_texto)
            else:
                surface_texto, rect_texto = text_objects(
                    "Turno do vermelho", fonte, VERMELHO_CLARO
                )
                rect_texto.center = (700, ALTURA / 2)
                display.blit(surface_texto, rect_texto)
        else:
...
def creditos():
    ...
    while not sair:
        ...
        surface_texto, rect_texto = text_objects(
            "Programador: Lucas de Medeiros", fonte, BRANCO
        )
        rect_texto.center = ((LARGURA / 2), ALTURA / 3)
        display.blit(surface_texto, rect_texto)

        surface_texto, rect_texto = text_objects(
            "Disciplina: Programacao 1 / Laboratorio de Programacao 1", fonte, BRANCO
        )
        rect_texto.center = ((LARGURA / 2), ALTURA / 2.7)
        display.blit(surface_texto, rect_texto)

        surface_texto, rect_texto = text_objects(
            "Versao Python: 2.7.x", fonte, VERMELHO_CLARO
        )
        rect_texto.center = ((LARGURA / 2), ALTURA / 1.5)
        display.blit(surface_texto, rect_texto)

        surface_texto, rect_texto = text_objects(
            "Versao Pygame: 1.9.1", fonte, VERMELHO_CLARO
        )
        rect_texto.center = ((LARGURA / 2), ALTURA / 1.3)
        display.blit(surface_texto, rect_texto)

        voltar = fonte.render(
            "Pressione qualquer tecla para voltar ao menu.", False, VERDE_CLARO
        )
...
def regras():
    ...
    while not sair:
        ...
        info1 = fonte.render(
            "O jogo de damas eh praticado em um tabuleiro de 64 casas.", False, (AZUL)
        )
        info2 = fonte.render(
            "O objetivo do jogo eh capturar todas as pecas do oponente.",
            False,
            (VERDE_ESCURO),
        )
        info3 = fonte.render(
            "A peca anda soh para frente, uma casa de cada vez, na diagonal.",
            False,
            (VERDE_ESCURO),
        )
        info4 = fonte.render(
            "Quando a peca atinge a oitava linha do tabuleiro ela vira dama.",
            False,
            (VERDE_ESCURO),
        )
        info5 = fonte.render(
            "A dama eh uma peca de movimentos mais amplos. Ela anda para frente e para tras,",
            False,
            (AZUL),
        )
        info6 = fonte.render(
            "quantas casas quiser, nao podendo saltar sobre uma peca da mesma cor. ",
            False,
            (AZUL),
        )
        info7 = fonte.render(
            "A captura e obrigatoria, ou seja, nao existe sopro.", False, (VERDE_ESCURO)
        )
        info8 = fonte.render(
            "Duas ou mais pecas juntas, na mesma diagonal, nao podem ser capturadas.",
            False,
            (VERDE_ESCURO),
        )
        info9 = fonte.render(
            "A peca e a dama podem capturar tanto para frente como para tras.",
            False,
            (AZUL),
        )
        info10 = fonte.render(
            "O movimento de captura pode ser encadeado sem que o jogador passe a vez.",
            False,
            (AZUL),
        )

        game1 = fonte.render(
            "Durante o jogo, ao clicar em uma peca, sera exibido em verde os movimentos",
            False,
            (VERMELHO),
        )
        game2 = fonte.render(
            "possiveis da mesma. Se nada acontecer ao clicar em uma peca, significa que",
            False,
            (VERMELHO),
        )
        game3 = fonte.render(
            "ela nao tem movimentos possiveis ou o turno pertence ao outro jogador.",
            False,
            (VERMELHO),
        )
...
def tela_vencedor(vencedor):
    ...
    while not sair:
        ...
        if vencedor == "empate":
            ...
            surface_texto, rect_texto = text_objects(
                "VITORIA DO  VERMELHO", fonte, VERMELHO
            )
        elif vencedor == "o":
...
def menu_jogo():
    while True:
        ...
        cria_botao(
            "MANUAL", (LARGURA - 560, ALTURA / 2, 120, 40), BRANCO, CINZA, PRETO, regras
        )
        cria_botao(
            "CREDITOS",
            (LARGURA - 360, ALTURA / 2, 120, 40),
            BRANCO,
            CINZA,
            PRETO,
            creditos,
